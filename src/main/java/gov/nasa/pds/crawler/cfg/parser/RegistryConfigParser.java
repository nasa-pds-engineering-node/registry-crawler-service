...
import gov.nasa.pds.crawler.cfg.model.RegistryCfg;
...
public class RegistryConfigParser
{
    public static RegistryCfg parseRegistry(Node root) throws Exception
    {
        ...
        if(count == 0) throw new Exception("Missing required element '/crawler/registry'");
        if(count > 1) throw new Exception("Could not have more than one '/crawler/registry' element");
