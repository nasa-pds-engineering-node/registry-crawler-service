...
/**
 * Crawler configuration file parser. Parses "/crawler/directories" section.
 * 
 * @author karpenko
 */
public class DirsParser
{
    public static List<String> parseDirectories(Node root) throws Exception
    {
        ...
        if(count > 1) throw new Exception("Could not have more than one '/crawler/directories' element");
    ...
    private static List<String> parsePaths(Node root) throws Exception
    {
        ...
        for(int i = 0; i < nl.getLength(); i++)
        {
            ...
            {
                throw new Exception("Invalid element '/crawler/directories/" + node.getNodeName() + "'");
            }
        }
        
        if(list.isEmpty())
        {
            throw new Exception("At least one '/crawler/directories/path' element is required.");
        }
